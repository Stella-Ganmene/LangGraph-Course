...
#    This causes the agent to re-call get_stale_leads after sending.
#    Since send_follow_up_email does NOT update last_contacted_at in 
#    the database, the same leads appear "stale" again → re-send.
#    Result: infinite email loop.
#
# 2. SECONDARY BUG: No recursion_limit on graph.compile().
#    Even if the logic loops, a recursion limit would stop it.
#
# 3. MISSING: No duplicate-send check in the send tool.
#
# FIXES NEEDED:
# - Add a tool to update last_contacted_at AFTER sending
# - OR remove the "check remaining" instruction from the prompt
# - Add recursion_limit=15 to graph.compile()
# - Add duplicate detection in send_follow_up_email
# ============================================================
