...
#
# 1. NO RECURSION LIMIT: graph.compile() has no recursion_limit.
#    The agent can loop forever.
#
# 2. NON-EXISTENT TABLE: get_lead_interactions queries a table 
#    called "interactions" that doesn't exist. It always fails.
#    But it returns a vague error, so the LLM thinks it should 
#    retry with different parameters.
#
# 3. UNRELIABLE TOOL: get_revenue_forecast has a 60% failure rate.
#    Combined with the prompt saying "retry until you get it,"
#    the agent keeps retrying indefinitely.
#
# 4. PROMPT ENCOURAGES PERSISTENCE: "Do not produce a partial 
#    report" + "If a tool call fails, try again" = the agent 
#    NEVER gives up. It keeps trying to get data from a table 
#    that doesn't exist and a service that mostly fails.
#
# THE LOOP:
#   get_all_leads → OK
#   get_lead_details(lead_1) → OK
#   get_lead_interactions(lead_1) → FAIL → retry → FAIL → retry...
#   (eventually gives up on this lead, moves to next)
#   get_lead_details(lead_2) → OK
#   get_lead_interactions(lead_2) → FAIL → retry → FAIL → retry...
#   ... (for each lead, multiple retries)
#   get_revenue_forecast → FAIL (60%) → retry → FAIL → retry...
#   "Report incomplete, need to retry everything" → starts over
#   → INFINITE LOOP
#
# FIXES NEEDED:
# 1. Add recursion_limit=25 to graph.compile()
# 2. Fix get_lead_interactions to return a CLEAR error:
#    "The interactions table does not exist. This data is unavailable."
# 3. Fix get_revenue_forecast to fail gracefully after 2 retries
# 4. Change prompt: "If a tool fails after 2 attempts, skip it and 
#    note the missing data in your report. A partial report is better 
#    than no report."
# 5. Add error counter to State to track consecutive failures
# ============================================================
